...
; Check if a list contains all q characuters
...
; Input:  L is a list of symbols (a,b,c,...,z).
; Output: a boolean value which is true when all of the elements in L
;         are eequal to q and false otherwise.
; The empty list should return true. Do you see why this base case makes sense?
(define (all_q_v2 L)
  (if (null? L)
      #t
      (foldr andi #t (map (lambda (x) (if (equal? 'q  x) #t #f)) L)))
)
...
(define (at_least_one_q_v2 L)
  (if (null? L)
      #t
      (foldr ori #f (map (lambda (x) (if (equal? x 'q) #t #f)) L)))
)
...
(define (odd_amt_q_v2 L)
  (if (null? L)
      #f
      (foldr xori #f (map (lambda (x) (if (equal? x 'q) #t #f)) L)))
)
