...
(define (get-variables expression)
  (cond
    ...
    [(not (list? expression)) '()]
    [(remove-duplicates (append(get-variables (first expression))(get-variables (rest expression))))]  
  )
)
...
(define (make_bindings name)
  (cons (cons (cons name (cons #t '())) '()) (cons (cons (cons name (cons #f '())) '() ) '()))
)
...
(define (insert_binding binding environments)
  (if (null? environments)
      '()
      (cons (append binding (first environments))(insert_binding binding (rest environments)))
  )
  )
...
(define (insert_multiple_bindings bindings environments)
  (cond
    ...
    [(null? bindings) null]
    [else
     (append (insert_binding (first bindings) environments) (insert_multiple_bindings (rest bindings) environments))]
    )
  )
